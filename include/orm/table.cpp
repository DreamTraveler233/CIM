...
namespace CIM::orm
{
    ...
    bool Table::init(const tinyxml2::XMLElement &node)
    {
        ...
        do
        {
            ...
            {
                CIM_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
        ...
        do
        {
            ...
            {
                CIM_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
            ...
            for (auto &x : cnames)
            {
                ...
                {
                    CIM_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
    ...
    {
        std::string p = path + "/" + replace(m_namespace, ".", "/");
        FSUtil::Mkdir(p);
        gen_inc(p);
        gen_src(p);
    }
    ...
    void Table::gen_inc(const std::string &path)
    {
        ...
        std::vector<std::string> ns = split(m_namespace, '.');
    ...
    std::string Table::genToStringSrc(const std::string &class_name)
    {
             ...
             it != m_cols.end(); ++it)
        {
            ...
            {
                ss << "Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
                   << ";" << std::endl;
            }
            else
            {
        ...
        ss << "    return JsonUtil::ToString(jvalue);" << std::endl;
        ss << "}" << std::endl;
        return ss.str();
    }
    ...
    void Table::gen_dao_inc(std::ofstream &ofs)
    {
        ...
        for (auto &i : m_idxs)
        {
            if (i->getDType() == Index::TYPE_UNIQ || i->getDType() == Index::TYPE_PK || i->getDType() == Index::TYPE_INDEX)
            {
                ofs << "    static int Delete";
                std::string tmp = "by";
