...
namespace CIM
{
    ...
    SmtpClient::ptr SmtpClient::Create(const std::string &host, uint32_t port, bool ssl)
    {
        ...
        if (!addr)
        {
            CIM_LOG_ERROR(g_logger) << "invalid smtp server: " << host << ":" << port
                                    << " ssl=" << ssl;
            return nullptr;
        }
        addr->setPort(port);
        ...
        {
            CIM_LOG_ERROR(g_logger) << "connect smtp server: " << host << ":" << port
                                    << " ssl=" << ssl << " fail";
            return nullptr;
        }
        std::string buf;
...
#define DO_CMD()                \
    ...
    {                           \
        ...
        if (!m_authed && !email->getFromEMailAddress().empty())
        {
            ...
            cmd = base64encode(email->getFromEMailAddress().substr(0, pos)) + "\r\n";
            DO_CMD();
