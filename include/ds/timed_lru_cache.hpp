...
namespace CIM::ds
{
    ...
    public:
        ...
        void set(const K &k, const V &v, uint64_t expired)
        {
            ...
            if (it != m_cache.end())
            {
                ...
                it->second->val = v;
                it->second->ts = expired + GetCurrentMS();
                m_timed.insert(it->second);
                return;
            }

            m_keys.emplace_front(Item(k, v, expired + GetCurrentMS()));
    ...
    template <class K, class V, class MutexType = Mutex, class Hash = std::hash<K>>
