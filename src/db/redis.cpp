...
namespace CIM
{
    ...
    static std::string get_value(const std::map<std::string, std::string> &m, const std::string &key, const std::string &def = "")
    {
        auto it = m.find(key);
    ...
    bool Redis::connect(const std::string &ip, int port, uint64_t ms)
    {
        ...
        if (c)
        {
            ...
            if (!m_passwd.empty())
            {
                ...
                if (!r)
                {
                    CIM_LOG_ERROR(g_logger) << "auth error:("
                                            << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (r->type != REDIS_REPLY_STATUS)
                {
                    CIM_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                                            << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                if (!r->str)
                {
                    CIM_LOG_ERROR(g_logger) << "auth reply str error: NULL("
                                            << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
                ...
                {
                    CIM_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                                            << m_host << ":" << m_port << ", " << m_name << ")";
                    return false;
                }
            }
    ...
    ReplyPtr Redis::cmd(const char *fmt, va_list ap)
    {
        ...
        if (!r)
        {
            if (m_logEnable)
            {
                CIM_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port << ")(" << m_name << ")";
            }
            return nullptr;
        }
    ...
    ReplyPtr Redis::cmd(const std::vector<std::string> &argv)
    {
        ...
        if (!r)
        {
            if (m_logEnable)
            {
                CIM_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
            }
            return nullptr;
        }
        ReplyPtr rt(r, freeReplyObject);
    ...
    ReplyPtr Redis::getReply()
    {
        ...
        if (m_logEnable)
        {
            CIM_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ...
    void FoxRedis::OnAuthCb(redisAsyncContext *c, void *rp, void *priv)
    {
        ...
        if (strcmp(r->str, "OK") == 0)
        {
            CIM_LOG_INFO(g_logger) << "auth ok: " << r->str << "("
                                   << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
        }
        else
        {
    ...
    void FoxRedis::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            CIM_LOG_INFO(g_logger) << "FoxRedis::ConnectCb "
        ...
        {
            CIM_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb "
                                    << c->c.tcp.host << ":" << c->c.tcp.port
                                    << " fail, error:" << c->errstr;
    ...
    {
        CIM_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb "
                               << c->c.tcp.host << ":" << c->c.tcp.port
                               << " status:" << status;
        FoxRedis *ar = static_cast<FoxRedis *>(c->data);
        ar->m_status = UNCONNECTED;
    ...
    bool FoxRedis::pinit()
    {
        ...
        if (!ctx)
        {
            CIM_LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port
                                    << ") null";
            return false;
        }
        if (ctx->err)
    ...
    void FoxRedis::Ctx::EventCb(int fd, short event, void *d)
    {
        ...
        if (ctx->rds->m_logEnable)
        {
            ...
            CIM_LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
                                   << (ctx->rds->m_cmdTimeout.tv_sec * 1000 + ctx->rds->m_cmdTimeout.tv_usec / 1000) << "ms";
        }
        if (ctx->fctx->fiber)
        {
            ctx->fctx->scheduler->schedule(&ctx->fctx->fiber);
    ...
    void FoxRedisCluster::ConnectCb(const redisAsyncContext *c, int status)
    {
        ...
        if (!status)
        {
            CIM_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb "
                                   << c->c.tcp.host << ":" << c->c.tcp.port
                                   << " success";
            if (!ar->m_passwd.empty())
            {
        ...
        {
            CIM_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb "
                                    << c->c.tcp.host << ":" << c->c.tcp.port
                                    << " fail, error:" << c->errstr;
        }
    }
    ...
    {
        CIM_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb "
                               << c->c.tcp.host << ":" << c->c.tcp.port
                               << " status:" << status;
    }
    ...
    bool FoxRedisCluster::pinit()
    {
        ...
        if (!ctx)
        {
            CIM_LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host
                                    << ") null";
            return false;
        }
        if (ctx->err)
    ...
    void FoxRedisCluster::pcmd(FCtx *fctx)
    {
        ...
        {
            CIM_LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected "
                                   << fctx->cmd;
            init();
            if (fctx->fiber)
            {
