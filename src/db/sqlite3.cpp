...
namespace CIM
{
    ...
    static ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_sqlite3_dbs = Config::Lookup("sqlite3.dbs", std::map<std::string, std::map<std::string, std::string>>(), "sqlite3 dbs");
    ...
    SQLite3::ptr SQLite3Manager::get(const std::string &name)
    {
        ...
        if (!sql.empty())
        {
            if (rt->execute(sql))
            {
                CIM_LOG_ERROR(g_logger) << "execute sql=" << sql
    ...
    SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string &name, bool auto_commit)
    {
        ...
        if (!conn)
        {
            CIM_LOG_ERROR(g_logger) << "SQLite3Manager::openTransaction, get(" << name
                                      << ") fail";
            return nullptr;
        }
