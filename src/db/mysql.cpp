...
namespace CIM
{
                             ...
                             const int &timeout)
    {
        ...
        {
            CIM_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
                                    << ", " << port << ", " << dbname
    ...
    int MySQL::execute(const char *format, va_list ap)
    {
        ...
        if (r)
        {
            CIM_LOG_ERROR(g_logger) << "cmd=" << cmd()
                                    << ", error: " << getErrStr();
            m_hasError = true;
        }
        else
    ...
    static MYSQL_RES *my_mysql_query(MYSQL *mysql, const char *sql)
    {
        ...
        {
            CIM_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
                                    << mysql_error(mysql);
            return nullptr;
        }
        ...
        if (res == nullptr)
        {
            CIM_LOG_ERROR(g_logger) << "mysql_store_result() error:"
                                    << mysql_error(mysql);
        }
        return res;
    }
    ...
    MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string &stmt)
    {
        ...
        {
            CIM_LOG_ERROR(g_logger) << "stmt=" << stmt
...
#undef XX
    ...
    MySQLTransaction::ptr MySQLManager::openTransaction(const std::string &name, bool auto_commit)
    {
        ...
        if (!conn)
        {
            CIM_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
                                    << ") fail";
            return nullptr;
        }
