...
namespace CIM
{
    ...
    static ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_thread_info_set = Config::Lookup("fox_thread", std::map<std::string, std::map<std::string, std::string>>(), "confg for thread");
    ...
    void FoxThreadManager::init()
    {
        ...
        for (auto i : m)
        {
            ...
            if (num <= 0)
            {
                CIM_LOG_ERROR(g_logger) << "thread pool:" << name
                                        << " num:" << num
                                        << " advance:" << advance
                                        << " invalid";
                continue;
            ...
            else
            {
                m_threads[i.first] = FoxThreadPool::ptr(new FoxThreadPool(
                    num, name, advance));
                CIM_LOG_INFO(g_logger) << "init thread pool:" << name
                                       << " num:" << num
                                       << " advance:" << advance;
            }
        }
