...
namespace CIM
{
    ...
    void ZKServiceDiscovery::start()
    {
        ...
        m_timer = IOManager::GetThis()->addTimer(60 * 1000, [self, this]()
                                                 {
        m_isOnTimer = true;
    ...
    bool ZKServiceDiscovery::existsOrCreate(const std::string &path)
    {
        ...
        else
        {
            ...
            if (pos == 0 || existsOrCreate(path.substr(0, pos)))
            {
                ...
                if (v != ZOK)
                {
                    CIM_LOG_ERROR(g_logger) << "create path=" << path << " error:"
                                            << zerror(v) << " (" << v << ")";
                    return false;
                }
                return true;
                                          ...
                                          const std::string &ip_and_port, const std::string &data)
    {
        ...
        if (!m_isOnTimer)
        {
            CIM_LOG_ERROR(g_logger) << "create path=" << (path + "/" + ip_and_port) << " fail, error:"
                                    << zerror(rt) << " (" << rt << ")";
        }
    ...
    bool ZKServiceDiscovery::queryInfo(const std::string &domain, const std::string &service)
    {
        if (service != "all")
        {
            ...
            if (!m_isOnTimer)
            {
                CIM_LOG_ERROR(g_logger) << "create path=" << (path + "/" + m_selfInfo) << " fail, error:"
                                        << zerror(rt) << " (" << rt << ")";
            }
            return rt == ZNODEEXISTS;
        }
    ...
    bool ZKServiceDiscovery::getChildren(const std::string &path)
    {
        ...
        {
            CIM_LOG_ERROR(g_logger) << "get_children path=" << path
                                    << " invalid path";
            return false;
        }
        {
        ...
        if (v != ZOK)
        {
            CIM_LOG_ERROR(g_logger) << "get_children path=" << path << " fail, error:"
                                    << zerror(v) << " (" << v << ")";
            return false;
        }
        ...
        for (auto &i : vals)
        {
            ...
            CIM_LOG_INFO(g_logger) << "domain=" << domain
    ...
    void ZKServiceDiscovery::onWatch(int type, int stat, const std::string &path, ZKClient::ptr client)
    {
        ...
        CIM_LOG_ERROR(g_logger) << "onWatch hosts=" << m_hosts
                                << " type=" << type << " stat=" << stat
