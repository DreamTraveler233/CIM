...
namespace CIM
{
    EMailEntity::ptr EMailEntity::CreateAttach(const std::string &filename)
    {
        ...
        entity->addHeader("Content-Type", "application/octet-stream;name=" + FSUtil::Basename(filename));
        return entity;
    }
    ...
    EMail::ptr EMail::Create(const std::string &from_address, const std::string &from_passwd, const std::string &title, const std::string &body, const std::vector<std::string> &to_address, const std::vector<std::string> &cc_address, const std::vector<std::string> &bcc_address)
    {
